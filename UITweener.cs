...
public class UITweener : MonoBehaviour
{
    ...
    void Scale()
    {
        var rt = _target.GetComponent<Transform>();
        if (_startOffset)
        {
            rt.localScale = _from;
        }
        ...
        _tweenObj = LeanTween.scale(_target, to, _duration);
    }
    ...
    void Rotate()
    {
        ...
        if (_startOffset)
        {
            t.localRotation = Quaternion.Euler(_from);
        }
        var to = _to;
        if (_animationMethod == UIAnimayionMethods.By)
        {
        ...
        _tweenObj = LeanTween.rotate(_target, to, _duration);
    }
    ...
    void RotateRT(RectTransform rt)
    {
        ...
        _tweenObj = LeanTween.rotateAroundLocal(_target, Vector3.forward, to.z, _duration);
    }
}